...
class ReceivableController {
    async list(req, res) {
        try {
            ...
            const receivables = await prisma_1.prisma.receivable.findMany({
                where: { companyId },
                include: {
    ...
    async markAsPaid(req, res) {
        try {
            ...
            if (!companyId)
                return res.status(401).json({ error: 'Não autorizado' });
            const receivable = await prisma_1.prisma.receivable.findFirst({ where: { id, companyId } });
            if (!receivable)
                return res.status(404).json({ error: 'Conta não encontrada' });
            const updated = await prisma_1.prisma.receivable.update({
                where: { id },
                data: {
                    status: 'PAID',
                    paidAt: new Date()
